...
class DecoderTransformerConfig:
    ...
    bias: bool = True
    is_causal: bool = True
    loss_fn: Callable = lambda x, y: F.cross_entropy(x.view(-1, x.shape[-1]), y.view(-1))
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        y = F.scaled_dot_product_attention(q, k, v, dropout_p=dropout_p, is_causal=self.is_causal)
...
class DecoderTransformerStack(nn.Module):
    ...
    def forward(self, x: Tensor, y=None, backward=False) -> Tensor:
...
class DecoderTransformer(nn.Module):
    ...
    def configure_optimizers(
        ...
        # only parameters that are 2D will have weight decayed.
